...
@Component({
  ...
  imports: [IonInput, IonButton, CommonModule, ReactiveFormsModule, MaskitoDirective],
})
export class PixKeyValidationComponent implements OnInit {
  ...
  public setType(val: 'cpf' | 'phone'): void {
    ...
    if (val === 'cpf') {
      this.validationFormGroup.controls['cpf'].addValidators([Validators.required]);
      this.validationFormGroup.controls['phone'].removeValidators([Validators.required]);
    } else {
      this.validationFormGroup.controls['cpf'].removeValidators([Validators.required]);
      this.validationFormGroup.controls['phone'].addValidators([Validators.required]);
    }
